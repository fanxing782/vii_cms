...
export function cssVariablesResolver(theme: MantineTheme) {
  ...
  for (const key of keys(theme.other.easings)) {
    result.variables[`--mantine-easing-${String(key)}`] = theme.other.easings[key]
  }

  for (const key of keys(theme.other.animations)) {
    result.variables[`--mantine-animation-${String(key)}`] = theme.other.animations[key]
  }

  for (const key of keys(theme.other.durations)) {
    result.variables[`--mantine-duration-${String(key)}`] = theme.other.durations[key]
  }

  for (const key of keys(theme.other.letterSpacings)) {
    result.variables[`--mantine-letter-spacing-${String(key)}`] = theme.other.letterSpacings[key]
  }

  for (const key of keys(theme.other.blurs)) {
    result.variables[`--mantine-blur-${String(key)}`] = theme.other.blurs[key]
  }

  for (const key of keys(theme.other.aspectRatios)) {
    result.variables[`--mantine-aspect-ratio-${String(key)}`] = theme.other.aspectRatios[key]
  }
